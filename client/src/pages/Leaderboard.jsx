...
import { LEADERBOARD_DATA, getInitials } from '../data/mockData'

const TIERS = [
  {
    title: 'Crimson Quest Master',
    range: 'Rank 1–10',
    desc: 'The undisputed few. They have seen the far reaches of the realm.',
    color: '#f59e0b',
  ...
  },
  {
    title: 'Quest Knight',
    range: 'Rank 11–25',
    desc: 'Battle-hardened veterans who know the weight of the sword.',
    color: '#9d5ef5',
...
function tierOf(rank) {
  if (rank <= 10) return TIERS[0]
  if (rank <= 25) return TIERS[1]
  return TIERS[2]
}
  ...
  return (
    <Layout>
      ...
      <p className="page-sub" style={{ marginBottom: 28 }}>
        The top 50 heralds of the realm — ranked by glory
      </p>
...
const S = {
  ...
  tierDesc:  { fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5 },
  table: {
  ...
  rowTop: { background: 'rgba(245,158,11,.04)' },
  thRank:   { width: 32, flexShrink: 0 },
  thHerald: { flex: 1 },
  thCenter: { width: 160, textAlign: 'center', flexShrink: 0 },
  thNum:    { width: 70, textAlign: 'right', flexShrink: 0 },
  rankNum:  { width: 32, fontSize: 13, fontWeight: 600, color: 'var(--text-muted)', flexShrink: 0 },
